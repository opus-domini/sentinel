...
func TestOpsRunbooksAndJobsHandlers(t *testing.T) {
	...
	select {
	case evt := <-eventsCh:
		if evt.Type != events.TypeOpsJob {
			t.Fatalf("unexpected runbook event type: %s", evt.Type)
		}
	case <-time.After(500 * time.Millisecond):
		t.Fatal("did not receive expected runbook event")
	}
...
// ---------------------------------------------------------------------------
// Ops handler tests – opsUnitStatus, opsUnitLogs errors
// ---------------------------------------------------------------------------
...
func TestFlushStorageNilRepo(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodPost, "/api/ops/storage/flush", strings.NewReader(`{"resource":"activity-journal"}`))
	h.flushStorage(w, r)
