...
func (h *Handler) registerTmuxRoutes(mux *http.ServeMux) {
	h.registerRoutes(mux, []routeBinding{
		...
		{pattern: "GET /api/tmux/launchers", handler: h.listTmuxLaunchers},
		{pattern: "POST /api/tmux/launchers", handler: h.createTmuxLauncher},
		{pattern: "PATCH /api/tmux/launchers/order", handler: h.reorderTmuxLaunchers},
		{pattern: "PATCH /api/tmux/launchers/{launcher}", handler: h.updateTmuxLauncher},
		{pattern: "DELETE /api/tmux/launchers/{launcher}", handler: h.deleteTmuxLauncher},
		{pattern: "POST /api/tmux/sessions/{session}/launchers/{launcher}/launch", handler: h.launchTmuxLauncher},
