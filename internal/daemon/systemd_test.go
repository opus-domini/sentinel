...
func TestStopUserAutoUpdateTimer(t *testing.T) {
	...
	// stopUserAutoUpdateTimer swallows errors, so all branches should complete.
	// These all call systemctl --user which may fail, but the function ignores errors.
	tests := []struct {
		name    string
		disable bool
		stop    bool
	}{
		{"disable and stop", true, true},
		{"disable only", true, false},
		{"stop only", false, true},
		{"noop", false, false},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			// This should not panic or error, just return
			stopUserAutoUpdateTimer(tc.disable, tc.stop)
		})
	}
}

func TestApplyUserAutoUpdateTimerState(t *testing.T) {
	t.Parallel()
	...
	// Test all branches. These call systemctl --user which will fail for
	// non-installed units, but we're testing the branch coverage.
	tests := []struct {
		name   string
		enable bool
		start  bool
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			err := applyUserAutoUpdateTimerState(tc.enable, tc.start)
			if tc.enable || tc.start {
				// Expect an error because the unit doesn't exist, but the function should not panic
				_ = err
			} else if err != nil {
				// noop should return nil
				t.Fatalf("noop should return nil: %v", err)
			}
		})
	}
}

func TestInstallUserOnLinuxNonRoot(t *testing.T) {
	t.Parallel()
	...
	// InstallUser with a valid exec path should proceed past the exec path check.
	// It will attempt to create the service file and daemon-reload.
	// We use a temp path as exec to keep the test clean.
	tmpDir := t.TempDir()
	fakeBin := filepath.Join(tmpDir, "sentinel")
	...
	if err != nil {
		// May fail on daemon-reload if user dbus is not accessible
		if strings.Contains(err.Error(), "systemctl") ||
			strings.Contains(err.Error(), "Failed to connect") ||
			strings.Contains(err.Error(), "bus") {
			return
...
func TestInstallUserAutoUpdateOnLinuxUserScope(t *testing.T) {
	t.Parallel()
	...
	// InstallUserAutoUpdate with user scope should attempt the full user install path
	tmpDir := t.TempDir()
	fakeBin := filepath.Join(tmpDir, "sentinel")
	if err := os.WriteFile(fakeBin, []byte("#!/bin/sh\n"), 0o755); err != nil { //nolint:gosec // test needs executable
		t.Fatalf("write fake binary: %v", err)
	...
	if err != nil {
		// May fail on daemon-reload or systemctl user bus
		if strings.Contains(err.Error(), "systemctl") ||
			strings.Contains(err.Error(), "Failed to connect") ||
			strings.Contains(err.Error(), "bus") {
			return
