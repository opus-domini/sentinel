...
describe('integrations draft', () => {
  it('builds a typed patch without putting the webhook secret in the diff', () => {
  ...
  it('requires the webhook replacement but does not parse cron in the browser', () => {
    ...
    ).toEqual({ webhook: 'Enter a non-empty replacement webhook URL.' })
  })
